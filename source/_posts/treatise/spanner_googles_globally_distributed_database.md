...
因此，在没有准备好的事务的情况下，空闲 Paxos 组中的健康从服务器可以在最坏的情况下提供大于 8 秒的 readat 时间戳。
领导节点也可以根据从属节点的要求推进 MinNextTS() 值。

##### 4.2.5 Paxos 领导者-租赁管理

确保 Paxos-leader-lease 间隔不相交的最简单方法是，无论何时，Leader 都会发出租用间隔的同步 Paxos 写入。随后的领导者将读取该间隔并等待该间隔过去。
