...
实验结果表明，与两种流行的消息系统相比，Kafka具有更好的性能。
...
然而，最近互联网应用的趋势使得活动数据成为生产数据管道的一部分，直接用于站点上的特定功能。
这些用途包括：
...
在第 3 节中，我们会描述 Kafka 的架构及其关键设计原则。
...
最重要的是，我们认为一个有用的流实用程序库，例如不同的窗口函数或连接技术，将对此类应用程序有益。
