...
注意当 $a = 2$ 时 (3.1.1) 的右边简化为

$$
1 / (Q_s (d-1) Q_s(d))
$$

如第 3 节所述，在 D 维网格上为 $O(d^{-2D})$。

本节和下一节中报告的模拟结果，对于选定的 a 值，要么采用均匀分布，要么采用上述形式的分布。
（为了避免 $Q(d) = 0$ 处的奇异性，我们在公式 (3.1.1) 中对 $Q(d)$ 加 1。）

表 4 总结了使用 `push-pull` 式反熵算法且无连接限制的 CIN 拓扑结构的结果。
该表表示 250 次运行，每次运行传播一个在随机选择的站点引入的更新。
“比较流量”数字表示每个周期的反熵比较次数，取所有网络链路的平均值，并分别取跨大西洋到英国布希链路的平均值。
“更新流量”表示必须从一个站点向另一个站点发送更新的反熵交换总数。
（如果使用校验和进行数据库对比，则比较流量和更新流量之间的区别可能很大，如第 1.3 节所述）

> 表 4. 反熵、无连接限制的模拟结果。

| Spatial Distribution | $t_last$ | $t_ave$ | Compare Traffic Average | Compare Traffic Bushey | Update Traffic Average | Update Traffic Bushey |
|----------------------|------------------|------------------|--------------------------|-------------------------|-------------------------|------------------------|
...
1. 将 $a = 2$ 的结果与均匀分布的情况进行比较，收敛速度 $t_last$ 下降了不到 2 倍，而每轮的平均流量提高了 4 倍以上。可以说，我们可以承受在非均匀分布下以两倍的频率执行反熵，从而获得相同的收敛速度，同时平均流量提高了两倍。
2. 再次将 $a = 2$ 的结果与均匀分布的情况进行比较，跨大西洋链路上的流量从 75.7 下降到 2.4，下降了 30 多倍。该链路上的流量现在不到平均值的两倍。我们认为这是非均匀分布对于 CIN 拓扑结构最重要的优势。使用这种分布，反熵交换不会使关键网络链路过载。

表 4 中的结果假设没有连接限制。
这个假设非常不现实——实际的 Clearinghouse 服务器一次只能支持少量的反熵连接。
表 5 给出了在最悲观的假设下的模拟结果：连接限制为 1，搜索限制为 0。
与上表相同，该表代表 250 次运行，每次运行传播一个在随机选择的站点引入的更新。

> 表 5. 反熵、连接限制为 1 时的模拟结果。
...
3. 表 5 中的“比较流量”数据明显低于表 4 中的数据，这反映出每个周期建立的成功连接数较少。表 5 中的收敛时间也相应地高于表 4 中的收敛时间。这些影响在分布不太均匀的情况下更为明显。
4. 当施加连接限制时，总比较流量（收敛时间与“比较流量”的乘积）不会发生显著变化；每个周期的比较流量会减少，而周期数会增加。

总结一下：使用具有反熵的空间分布可以显著减少关键链路上的流量，而这些链路在采用均匀分布时会成为“热点”。
最悲观的连接限制会减慢收敛速度，但不会显著改变分发更新时产生的总流量；它只是在一定程度上减慢了更新的分发速度。

基于我们早期的模拟结果，我们在 Clearinghouse 服务的内部版本中实现了一个基于 $1/ Q(d)^2$  分布的非均匀反熵算法。
新版本现已安装到整个 CIN，并显著提升了 Clearinghouse 服务器的网络负载及其数据库的一致性。
