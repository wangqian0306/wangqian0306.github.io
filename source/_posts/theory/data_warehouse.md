...
- 将数据进行分块
  - (在从数据源读取的时候要注意将数据依照处理单元的内存大小或数据行数或查询事务进行分解)
  - 然后将其写入缓存库中，使其更有弹性
- 将数据进行转换
  - 将原始数据按照特点进行类型转换
- 避免循环
...
- 关注数据模型
  - 工作流出错大概率是源自数据不完整或格式错误，确保在输入时每个元素都要是正确的
- 建立重试逻辑
  - 不推荐因为想要重用元素而进行复用不通阶段的代码，最好的设计思路应该是将 ETL 设计为独立的部分从而进行重试
- 检查点机制
  - 在程序执行后写入执行日志明确数据处理位置